...
keys.clientkeys = gears.table.join(
    ...
    awful.key({ superkey }, "c", function (c)
        ...
        helpers.single_double_tap(
            nil,
            function ()
                helpers.float_and_resize(c, awful.screen.focused().geometry.width * 0.65, awful.screen.focused().geometry.height * 0.9)
            end)
    end),
    ...
    awful.key({ superkey, ctrlkey  }, "f",
        function (c)
            helpers.float_and_resize(c, awful.screen.focused().geometry.width * 0.7, awful.screen.focused().geometry.height * 0.75)
        end,
        {description = "focus mode", group = "client"}),
    -- V for vertical view
    awful.key({ superkey, ctrlkey  }, "v",
        function (c)
            helpers.float_and_resize(c, awful.screen.focused().geometry.width * 0.45, awful.screen.focused().geometry.height * 0.90)
        end,
        {description = "focus mode", group = "client"}),
    -- T for tiny window
    awful.key({ superkey, ctrlkey  }, "t",
        function (c)
            helpers.float_and_resize(c, awful.screen.focused().geometry.width * 0.3, awful.screen.focused().geometry.height * 0.35)
        end,
        {description = "tiny mode", group = "client"}),
    -- N for normal size (good for terminals)
    awful.key({ superkey, ctrlkey  }, "n",
        function (c)
            helpers.float_and_resize(c, awful.screen.focused().geometry.width * 0.45, awful.screen.focused().geometry.height * 0.5)
        end,
